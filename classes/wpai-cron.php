...
if (!class_exists('WPAI_Cron')) {
     ...
     */
    class WPAI_Cron extends WPAI_Module
    {
        ...
        {
            $schedules['wpai_debug'] = array(
                'interval' => 5,
                'display' => 'Every 5 seconds'
            );

            $schedules['wpai_ten_minutes'] = array(
                'interval' => 60 * 10,
                'display' => 'Every 10 minutes'
            );

            $schedules['wpai_example_interval'] = array(
                'interval' => 60 * 60 * 5,
        ...
        public static function fire_job_at_time()
        {
            ...
            if ((int)date('G', $now) >= 1 && (int)date('G', $now) <= 3) {
                if (!get_transient('wpai_cron_example_timed_job')) {
                    //WPAI_CPT_Example::exampleTimedJob();
                    set_transient('wpai_cron_example_timed_job', true, 60 * 60 * 6);
                }
            }
        }
        ...
        {
            add_action('wpai_cron_timed_jobs', __CLASS__ . '::fire_job_at_time');
            add_action('wpai_cron_example_job', __CLASS__ . '::example_job');
        ...
        {
            if (wp_next_scheduled('wpai_cron_timed_jobs') === false) {
                wp_schedule_event(
                    current_time('timestamp'),
                    'wpai_ten_minutes',
                    'wpai_cron_timed_jobs'
                );
            }

            if (wp_next_scheduled('wpai_cron_example_job') === false) {
                wp_schedule_event(
                    current_time('timestamp'),
                    'wpai_example_interval',
                    'wpai_cron_example_job'
                );
            }
        }
        ...
        {
            wp_clear_scheduled_hook('wpai_timed_jobs');
            wp_clear_scheduled_hook('wpai_example_job');
        }
